...
protected func HandleDamagePreview( weapon : WeaponObject, scriptInterface : StateGameScriptInterface, stateContext : StateContext )
{
...
protected func OnEnter( stateContext : StateContext, scriptInterface : StateGameScriptInterface )
{
...
protected func ConsumeStamina( scriptInterface : StateGameScriptInterface, attackRecord : wref< Attack_Record >, staminaPenaltyMultiplier : Float, staminaFullChargePenaltyMultiplier : Float )
{
...
@wrapMethod(AimingStateEvents)
protected func OnExit( stateContext : StateContext, scriptInterface : StateGameScriptInterface )
{
  ...
  if (!Equals(FocusSE, null))
  {
    if ( FocusSE.GetRemainingDuration() < (FocusSE.GetTotalDuration()-focusGracePeriod) )
    {
      ...
      PlayerStaminaHelpers.ModifyStamina( scriptInterface.executionOwner as PlayerPuppet, - TDB.GetFloat( t"NewPerks.Cool_Left_Milestone_2.focusedStaminaCost" ) );
    }
  }
}
...
public func OnItemEquipped( slot : TweakDBID, item : ItemID )
{
  ...
  let triggerModes : array<TriggerMode_Record> = this.m_weapon.GetTriggerModes();
...
private static func IsWreckingBallAllowed( const scriptInterface : StateGameScriptInterface ) -> Bool
{
...
protected const func EnterCondition( const stateContext : StateContext, const scriptInterface : StateGameScriptInterface ) -> Bool
{
...
public static func MeleeSprintStateCondition( const stateContext : StateContext, const scriptInterface : StateGameScriptInterface ) -> Bool
{
...
private static func CanBlockBullet( hitEvent : gameHitEvent ) -> Bool
{
  let defenderWeapon : WeaponObject;
